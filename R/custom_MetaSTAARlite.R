#' Performs meta-analysis of custom variant-sets (masks) using MetaSTAARlite
#'
#' This function performs meta-analysis to detect associations between a
#' quantitative/dichotomous phenotype and a user-specified custom mask by using the MetaSTAARlite pipeline.
#' For each custom mask, the MetaSTAAR-O p-value is a p-value from an omnibus test
...
#' specified in \code{\link{custom_MetaSTAARlite_worker}} (default = FALSE).
...
#' @return a list of data frames containing the MetaSTAAR p-values (including MetaSTAAR-O) corresponding to the custom mask.
